...
const checkAll = async () => {
    await db.none(`DO $$ BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'roles') THEN
            CREATE TYPE roles AS ENUM ('PROJECT_ADMIN', 'PROJECT_DESIGNER', 'VISITOR', 'SUPER_ADMIN');
        END IF;
    END $$;`)
    await db.none('CREATE TABLE IF NOT EXISTS users (user_id serial PRIMARY KEY, name VARCHAR ( 50 ) NOT NULL, email VARCHAR ( 255 ) UNIQUE NOT NULL, password VARCHAR ( 500 ) NOT NULL, setting JSONB NOT NULL, created_on TIMESTAMP NOT NULL, last_login TIMESTAMP, role roles NOT NULL DEFAULT \'VISITOR\')')
    ...
    await db.none("CREATE TABLE IF NOT EXISTS drafts_users (draft_id integer NOT NULL, user_id integer NOT NULL, created_by integer NOT NULL, created_on TIMESTAMP NOT NULL, updated_on TIMESTAMP NOT NULL, PRIMARY KEY (draft_id, user_id), FOREIGN KEY (draft_id) REFERENCES drafts (draft_id) ON DELETE CASCADE, FOREIGN KEY (user_id) REFERENCES users (user_id) ON DELETE CASCADE, FOREIGN KEY (created_by) REFERENCES users (user_id) ON DELETE CASCADE)");
...
const initData = async () => {
    ...
    try {
      await initialCheck('SELECT * FROM users WHERE user_id = $1', ['1'], 'INSERT INTO users(user_id, name, email, password, setting, role, created_on) VALUES($1, $2, $3, $4, $5, $6, current_timestamp) ON CONFLICT DO NOTHING', ['1', 'sinotech', process.env.ADMIN_EMAIL, hashed, { admin: true }, 'SUPER_ADMIN'])
